...
func GetUserRoleDB(ctx context.Context, defDB *gorm.DB) *gorm.DB {
	return util.GetDB(ctx, defDB).Model(new(model.UserRole))
}
...
func (a *UserRole) Query(ctx context.Context, params model.UserRoleQueryParam, opts ...model.UserRoleQueryOptions) (*model.UserRoleQueryResult, error) {
	var opt model.UserRoleQueryOptions
	if len(opts) > 0 {
		opt = opts[0]
	}

	db := a.DB.Table(fmt.Sprintf("%s AS a", new(model.UserRole).TableName()))
	if opt.JoinRole {
	...
	queryResult := &model.UserRoleQueryResult{
...
func (a *UserRole) Get(ctx context.Context, id string, opts ...model.UserRoleQueryOptions) (*model.UserRole, error) {
	var opt model.UserRoleQueryOptions
	if len(opts) > 0 {
		opt = opts[0]
	}
...
func (a *UserRole) Create(ctx context.Context, item *model.UserRole) error {
	result := GetUserRoleDB(ctx, a.DB).Create(item)
	return errors.WithStack(result.Error)
}

// Update the specified user role in the database.
func (a *UserRole) Update(ctx context.Context, item *model.UserRole) error {
...
func (a *UserRole) Delete(ctx context.Context, id string) error {
	result := GetUserRoleDB(ctx, a.DB).Where("id=?", id).Delete(new(model.UserRole))
	return errors.WithStack(result.Error)
}

func (a *UserRole) DeleteByUserID(ctx context.Context, userID string) error {
	result := GetUserRoleDB(ctx, a.DB).Where("user_id=?", userID).Delete(new(model.UserRole))
	return errors.WithStack(result.Error)
}

func (a *UserRole) DeleteByRoleID(ctx context.Context, roleID string) error {
	result := GetUserRoleDB(ctx, a.DB).Where("role_id=?", roleID).Delete(new(model.UserRole))
	return errors.WithStack(result.Error)
}
