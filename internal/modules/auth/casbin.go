...
func (a *Casbinx) load(ctx context.Context) error {
	start := time.Now()
	roleResult, err := a.RoleRepo.Query(ctx, model.RoleQueryParam{
		Status: model.RoleStatusEnabled,
	}, model.RoleQueryOptions{
...
func (a *Casbinx) queryRoleResources(ctx context.Context, roleID string) (model.MenuResources, error) {
	menuResult, err := a.MenuRepo.Query(ctx, model.MenuQueryParam{
		RoleID: roleID,
	...
	menuResourceResult, err := a.MenuResourceRepo.Query(ctx, model.MenuResourceQueryParam{
		MenuIDs: menuIDs,
	})
	if err != nil {
		return nil, err
